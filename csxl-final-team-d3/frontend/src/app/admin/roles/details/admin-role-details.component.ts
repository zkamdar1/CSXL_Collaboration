...
export class AdminRoleDetailsComponent implements OnInit {
  ...
  public onGrantPermission() {
    if (
      this.grantPermissionForm.action === '' ||
      this.grantPermissionForm.resource === ''
    ) {
      return;
    }
    this.roleAdminService
      .grant(this.role.id, this.grantPermissionForm)
      .subscribe((role: RoleDetails) => {
        this.role = role;
  ...
  public onAddMember() {
    ...
    this.roleAdminService
      .add(this.role.id, this.selectedUser)
      .subscribe((role: RoleDetails) => {
        this.role = role;
